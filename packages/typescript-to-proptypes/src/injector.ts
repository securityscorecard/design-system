          ...
          if (
            (node
              ...
              : babelTypes.isMemberExpression(path.node.object) &&
                ...
                babelTypes.isIdentifier(path.node.object.object.property, {
                  name: 'props',
                })) &&
...
function plugin(
  ...
  const shouldInclude: Exclude<InjectOptions['shouldInclude'], undefined> = (
    data,
  ) => {
  ...
  function injectPropTypes(injectOptions: {
    ...
    const source = generate(props, {
      ...
      reconcilePropTypes,
      shouldInclude: (prop) =>
        shouldInclude({ component: props, prop, usedProps }),
    });
    ...
    if (originalPropTypesPath !== null) {
      originalPropTypesPath.replaceWith(
        babel.template.ast(placeholder) as babelTypes.Statement,
      );
    } else if (
      !emptyPropTypes &&
        ...
        enter(path, state: any) {
          ...
          path.get('body').forEach((nodePath) => {
            ...
            if (
              ...
              babelTypes.isIdentifier(node.expression.left.property, {
                name: 'propTypes',
              })
            ) {
              ...
              if (babelTypes.isObjectExpression(maybeObjectExpression)) {
                ...
                maybeObjectExpression.properties.forEach((property) => {
                  if (babelTypes.isObjectProperty(property)) {
                    const validatorSource = code.slice(
                      property.value.start,
                      property.value.end,
                    );
                    if (babelTypes.isIdentifier(property.key)) {
                      previousPropTypesSource.set(
                        property.key.name,
                        validatorSource,
                      );
      ...
      VariableDeclarator(path) {
        ...
        function getFromProp(propsNode: babelTypes.Node) {
          ...
          injectPropTypes({
            ...
            usedProps:
              babelTypes.isIdentifier(propsNode) ||
              babelTypes.isObjectPattern(propsNode)
        ...
        if (
          ...
          if (
            ...
            (nodeInit.callee as babel.types.Identifier)?.name?.match(
              /create[A-Z].*/,
            )
          ) {
