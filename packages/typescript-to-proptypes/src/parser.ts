...
export function parseFromProgram(
  ...
  function isTypeJSXElementLike(type: ts.Type): boolean {
    if (type.isUnion()) {
      return type.types.every(
        (subType) =>
          subType.flags & ts.TypeFlags.Null || isTypeJSXElementLike(subType),
      );
    }
    if (type.symbol) {
          ...
          if (filtered.length > 0) {
            return t.createInterfaceType({
              ...
              types: filtered.map((x) => {
                ...
                const definition = checkSymbol(x, location, [
                  ...typeStack,
                  (type as any).id,
                ]);
  ...
  function checkSymbol(
    ...
    const isOptional =
      (symbol.flags & ts.SymbolFlags.Optional) === ts.SymbolFlags.Optional;
    ...
    if (declaration === undefined) {
      ...
      const baseConstraintOfType =
        checker.getBaseConstraintOfType(declaredType);

      if (
        baseConstraintOfType === undefined ||
        baseConstraintOfType === declaredType
      ) {
        type = symbolType;
      }
      // get `React.ElementType` from `C extends React.ElementType`
      else if (
        baseConstraintOfType.aliasSymbol?.escapedName === 'ElementType'
      ) {
  ...
  function parsePropsSymbol(
    ...
    const properties = type
      .getProperties()
      .filter((property) =>
        shouldInclude({ name: property.getName(), depth: 1 }),
      );
    // .map((property) => {
    //   const isOptional =
    ...
    const propsFilename =
      propsSourceFile !== undefined ? propsSourceFile.fileName : undefined;

    programNode.body.push(
      t.createComponent(
        name,
        properties.map((property) =>
          checkSymbol(property, location, [(type as any).id]),
        ),
        propsFilename,
      ),
    );
        ...
        componentNode.types.forEach((typeNode) => {
          ...
          if (currentTypeNode === undefined) {
            ...
            currentTypeNode = t.createPropTypeDefinition(
              ...
              new Set(
                Array.from(currentTypeNode.filenames).concat(
                  Array.from(typeNode.filenames),
                ),
              ),
              undefined,
            );
        ...
        if (ts.isVariableDeclaration(variableNode) && variableNode.name) {
          ...
          if (!variableNode.initializer) {
            if (
              ...
              checker.getFullyQualifiedName(type.aliasSymbol) ===
          ...
          if (checkDeclarations && variableNode.initializer) {
            if (
              type
                .getCallSignatures()
                .some((signature) =>
                  isTypeJSXElementLike(signature.getReturnType()),
                )
            ) {
              parseFunctionComponent(variableNode);
            }
