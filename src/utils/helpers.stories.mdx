...
#### createMarginSpacing, createPaddingSpacing
Generates spacing values for given kind.

```js
import { createMarginSpacing, createPaddingSpacing } from '@securityscorecard/design-system';
...
Returns border-radius from theme.
...
Returns modal width from theme for size given in `size` property.

```js
import { getModalSize } from '@securityscorecard/design-system';

// getModalSize :: Props -> string
// Props - styled-components props object
const Component = styled.div.attrs(() => ({size: 'lg'}))`
  width: ${getModalSize};
`; // -> width: 60.625rem;
```
