...
import { Grades, HexGradeProps } from './HexGrade.types';
...
const HexGrade = ({
  ...
  return (
    <StyledSVG
      className={cls(CLX_COMPONENT, className, { dark: isInverted })}
      data-grade={grade}
      height={size}
      margin={margin}
      viewBox="0 0 64 64"
    ...
    >
      <path
        d={wrappers[variant]}
        style={{
          fill: `var(--sscds-color-grade-${getDefiniteGrade(grade)}-${
      ...
      {typeof grade !== 'undefined' && (
        <path
          d={grades[grade].path}
          style={{
            fill: `var(--sscds-color-grade-${getDefiniteGrade(grade)}-char)`,
          }}
          transform={
            variant === HexGradeVariants.cs
              ? grades[grade].csTransform
              : undefined
          }
        />
      )}
