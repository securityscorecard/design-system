...
import type { Grades, HexGradeProps, HexGradeVariants } from './HexGrade.types';
import { createMarginSpacing } from '../../utils';
import { CLX_COMPONENT } from '../../theme/constants';
import { DSContext } from '../../theme/DSProvider/DSProvider';

const grades: Record<Grades, { csTransform: string; path: string }> = {
  '?': {
...
const wrappers: Record<HexGradeVariants, string> = {
  solid:
    'M 32.004 64.003 L 4.297 48.005 L 4.297 16.014 L 32.004 0.016 L 59.712 16.014 L 59.712 48.005 Z',
  outline:
    'M 32.004 64.003 L 4.297 48.005 L 4.297 16.014 L 32.004 0.016 L 59.712 16.014 L 59.712 48.005 Z M 54.712 18.9 L 32.004 5.79 L 9.297 18.9 L 9.297 45.119 L 32.004 58.229 L 54.712 45.119 Z',
  cs: 'M5 14.7017V42.334L11 46.029V18.165L33.0606 4.5792L27.2487 1L5 14.7017ZM15 48.334V20.7017L37.2487 7L59.4974 20.7017V48.334L37.2487 62.0357L15 48.334Z',
} as const;
...
const getDefiniteGrade = (
  grade: Grades,
): Omit<Grades, '?' | 'n/a'> | 'unknown' =>
  ['A', 'B', 'C', 'D', 'F'].includes(grade) ? grade : 'unknown';

const getShapeColor = (
  variant: HexGradeVariants,
  grade: Grades,
  hasLegacyColors: boolean,
  isInverted: boolean,
) => {
  const definiteGrade = getDefiniteGrade(grade);
  if (hasLegacyColors) {
    if (isInverted)
      return `var(--LEGACY_sscds-color-grade-${definiteGrade}-char)`;
    if (variant === 'outline')
      return `var(--LEGACY_sscds-color-grade-${definiteGrade}-stroke)`;
    return `var(--LEGACY_sscds-color-grade-${definiteGrade}-fill)`;
  }

  return `var(--sscds-color-grade-${definiteGrade}-${
    variant === 'outline' ? 'stroke' : 'fill'
  })`;
};
const getCharacterColor = (
  ...
  if (hasLegacyColors) {
    if (isInverted)
      return `var(--LEGACY_sscds-color-grade-${definiteGrade}-${
        variant === 'outline' ? 'char' : 'fill'
      })`;

    return `var(--LEGACY_sscds-color-grade-${definiteGrade}-${
      variant === 'outline' ? 'fill' : 'char'
    })`;
  }

  return `var(--sscds-color-grade-${definiteGrade}-char`;
};
  ...
  return (
    <StyledSVG
      className={cls(CLX_COMPONENT, className, {
        dark: !hasLegacyColors && isInverted,
      })}
      data-grade={grade}
      height={size}
      margin={margin}
    ...
    >
      ...
      {typeof grade !== 'undefined' && (
        <path
          ...
          transform={variant === 'cs' ? grades[grade].csTransform : undefined}
        />
      )}
    </StyledSVG>
  );
