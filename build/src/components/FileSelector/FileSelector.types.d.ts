...
declare type CustomFileSelectorProps = {
    /**
     * Button label text
     */
    buttonLabel?: string;
...
declare type BaseFileSelectorProps = React.HTMLAttributes<HTMLDivElement> & Omit<DropzoneOptions, 'disabled' | 'preventDropOnDocument' | 'noClick' | 'noDrag' | 'noKeyboard' | 'noDragEventsBubbling' | 'getFilesFromEvent' | 'useFsAccessApi' | 'minSize' | 'maxSize' | 'onDrop' | 'onDropAccepted' | 'onDropRejected'> & CustomFileSelectorProps;
export declare type FileSelectorProps = ({
    size?: typeof FileSelectorSizes.compact;
} & BaseFileSelectorProps) | ({
    size?: typeof FileSelectorSizes.fill;
} & BaseFileSelectorProps) | ({
    /**
